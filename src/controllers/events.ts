...
import { createEvent as createNewEvent, deleteEventById } from "../config/sqlQueries";
...
export const createEvent: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        const eventId:number | null = await db.oneOrNone(createNewEvent,[body.name, body.date, body.winningTeamId], r => r.event_id)
...
export const deleteEvent: RequestHandler<{id: number}> = async (req, res, next) => {
    try {
        ...
        const rowCount = await db.result(deleteEventById, [eventId], r => r.rowCount)
        if(rowCount === 0) {
